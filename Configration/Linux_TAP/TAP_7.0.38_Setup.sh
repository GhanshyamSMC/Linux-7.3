...
Now_Menu_BROKERID() {	
	...
	break; >>$LOG
       ...
       if [ $CM_UPGRADE_FLAG -eq 1 -a $CD_UPGRADE_FLAG -eq 1 -a $FO_UPGRADE_FLAG -eq 1 ];then #-a $FO_UPGRADE_FLAG
                echo
	...
	do	
		...
		read choice
		echo "$choice ">>$LOG
		if [ $choice -eq 4 -o $choice -eq 5 -o $choice -gt 6  ]; then
...
if [ $CD_INSTALL_FLAG = true ];then
	dos2unix $SetupFilesPath/exe/TAP_CD_7.0.28_Setup.sh 2>>$LOG
	bash $SetupFilesPath/exe/TAP_CD_7.0.28_Setup.sh $MULTI_TAP $InstallDir
	value=$?
	if [ $value -eq 0 ]; then
