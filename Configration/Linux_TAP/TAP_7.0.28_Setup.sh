...
Now_Menu_BROKERID() {	
	...
	break; >>$LOG
       ...
       if [ $CM_UPGRADE_FLAG -eq 1 -a $FO_UPGRADE_FLAG -eq 1 -a $CD_UPGRADE_FLAG -eq 1 ];then
                echo
...
if [ $CM_INSTALL_FLAG = true ];then
	dos2unix $SetupFilesPath/exe/TAP_CM_7.0.24_Setup.sh 2>>$LOG
	bash $SetupFilesPath/exe/TAP_CM_7.0.24_Setup.sh $MULTI_TAP $InstallDir
	value=$?
	if [ $value -eq 0 ]; then
...
if [ $FO_INSTALL_FLAG = true ];then
	dos2unix $SetupFilesPath/exe/TAP_FO_7.0.26_Setup.sh 2>>$LOG
	bash $SetupFilesPath/exe/TAP_FO_7.0.26_Setup.sh $MULTI_TAP $InstallDir
	value=$?
	if [ $value -eq 0 ]; then
...
if [ $CD_INSTALL_FLAG = true ];then
	dos2unix $SetupFilesPath/exe/TAP_CD_7.0.24_Setup.sh 2>>$LOG
	bash $SetupFilesPath/exe/TAP_CD_7.0.24_Setup.sh $MULTI_TAP $InstallDir
	value=$?
	if [ $value -eq 0 ]; then
